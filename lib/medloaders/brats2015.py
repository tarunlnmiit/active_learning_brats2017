...
class MICCAIBraTS2015(Dataset):
    """
    Code for reading the infant brain MICCAIBraTS2015 challenge
    """
    ...
    def __init__(self, args, mode, dataset_path='./datasets', classes=5, crop_dim=(32, 32, 32), split_idx=10,
        ...
        self.training_path = self.root + '/brats2015/train/'
        ...
        self.save_name = self.root + '/brats2015/train/brats2015-list-' + mode + '-samples-' + str(
            samples) + '.txt'
        ...
        if load:
            ...
            list_IDsT1 = sorted(glob.glob(os.path.join(self.training_path, '*GG/*/*T1.*/*.mha')))
        ...
        self.sub_vol_path = self.root + '/brats2015/train/generated/' + mode + subvol + '/'
        utils.make_dirs(self.sub_vol_path)

        list_IDsT1 = sorted(glob.glob(os.path.join(self.training_path, '*GG/*/*T1.*/*.mha')))
        list_IDsT1ce = sorted(glob.glob(os.path.join(self.training_path, '*GG/*/*T1c.*/*.mha')))
        list_IDsT2 = sorted(glob.glob(os.path.join(self.training_path, '*GG/*/*T2.*/*.mha')))
        list_IDsFlair = sorted(glob.glob(os.path.join(self.training_path, '*GG/*/*Flair.*/*.mha')))
        labels = sorted(glob.glob(os.path.join(self.training_path, '*GG/*/*.OT.*/*.mha')))
        ...
        if self.mode == 'train':
            ...
            self.list = create_sub_volumes(list_IDsT1, list_IDsT1ce, list_IDsT2, list_IDsFlair, labels,
                                           dataset_name="brats2015", mode=mode, samples=samples,
            ...
            self.list_IDsT1 = sorted(glob.glob(os.path.join(self.testing_path, '*GG/*/*T1.*/*.mha')))
            self.list_IDsT1ce = sorted(glob.glob(os.path.join(self.testing_path, '*GG/*/*T1c.*/*.mha')))
            self.list_IDsT2 = sorted(glob.glob(os.path.join(self.testing_path, '*GG/*/*T2.*/*.mha')))
            self.list_IDsFlair = sorted(glob.glob(os.path.join(self.testing_path, '*GG/*/*Flair.*/*.mha')))
            self.labels = None
