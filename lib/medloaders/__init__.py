...
from .brats2015 import MICCAIBraTS2015
from .brats2017 import MICCAIBraTS2017
...
def generate_datasets(args, path='.././datasets'):
    ...
    elif args.dataset_name == "brats2015":
        total_data = 244
        split_idx = int(split_percent * total_data)
        train_loader = MICCAIBraTS2015(args, 'train', dataset_path=path, classes=args.classes, crop_dim=args.dim, split_idx=split_idx, samples=samples_train, load=args.loadData)

        val_loader = MICCAIBraTS2015(args, 'test', dataset_path=path, classes=args.classes, crop_dim=args.dim, split_idx=split_idx, samples=samples_val, load=args.loadData)


    elif args.dataset_name == "brats2017":
        # print(args.loadData)
        total_data = 285

        split_idx = int(split_percent * total_data)

        train_loader = MICCAIBraTS2017(args, 'train', dataset_path=path, classes=args.classes, crop_dim=args.dim,

                                       split_idx=split_idx, samples=samples_train, load=args.loadData)

        val_loader = MICCAIBraTS2017(args, 'val', dataset_path=path, classes=args.classes, crop_dim=args.dim,
...
def select_full_volume_for_infer(args, path='.././datasets'):
    ...
    if args.dataset_name == "iseg2017":
        ...
        total_data = 244
        split_idx = int(split_percent * total_data)
        train_loader = MICCAIBraTS2015('train', dataset_path=path, classes=args.classes, crop_dim=args.dim,
                                       split_idx=split_idx, samples=samples_train)

        val_loader = MICCAIBraTS2015('val', dataset_path=path, classes=args.classes, crop_dim=args.dim,
                                     split_idx=split_idx,
                                     samples=samples_val)
        ...
        train_loader = MICCAIBraTS2017('train', dataset_path=path, classes=args.classes, crop_dim=args.dim,

                                       split_idx=split_idx, samples=samples_train)

        val_loader = MICCAIBraTS2017('val', dataset_path=path, classes=args.classes, crop_dim=args.dim,
